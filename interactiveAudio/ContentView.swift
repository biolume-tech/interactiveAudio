...
struct ContentView: View {
    ...
    // This establishes a source of truth for the app's state that can interact with SwiftUI's lifecycle, allowing the UI to react to changes in audio processing.
    ...
    var body: some View {
        ...
        VStack {
            ...
            Circle()
                ...
                // The hue parameter of the Color is dynamically bound to the highBand value, with full saturation and brightness, creating a color that changes with the audio's high frequency amplitude.
                .fill(Color(hue: Double(audioProcessor.highBand), saturation: 1.0, brightness: 1.0))
                ...
                .frame(width: self.mapAmplitudeToSize(amplitude: audioProcessor.lowBand), height: self.mapAmplitudeToSize(amplitude: audioProcessor.lowBand))
                .padding()
