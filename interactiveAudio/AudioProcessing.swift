...
// Define a class responsible for processing audio. This class adheres to ObservableObject to enable UI updates in SwiftUI based on changes in audio processing results.
class AudioEngineProcessor: ObservableObject {
    ...
    // Bandpass filters for isolating specific frequency bands. These are implemented as AVAudioUnitEQ with a single band each for low and high frequencies.
    var lowBandPassFilter: AVAudioUnitEQ
    var highBandPassFilter: AVAudioUnitEQ
    
    // Published properties for SwiftUI to react to changes. These represent the processed amplitude values for low and high frequency bands.
    ...
    // Configures the audio engine used for playback. This involves attaching the audio player node to the engine and connecting it to the engine's main mixer node.
    ...
    // Sets up the audio engine for analysis. This involves attaching the analysis player node and the bandpass filters to the engine, and connecting these components in a manner that allows for audio analysis.
    private func setupAudioEngineAnalysis() {
        ...
        // Define the audio format based on the analysis player node's output. This ensures compatibility throughout the audio processing chain.
        let audioFormat = analysisPlayerNode.outputFormat(forBus: 0)
        // Connect the analysis player node to both bandpass filters, and then to the engine's main mixer. This establishes a path for audio to be analyzed and filtered.
    ...
    private func setupLowBandPassFilter() {
        ...
        bandPass.frequency = 5 // Target frequency for the low band, isolating lower frequencies.
    ...
    private func setupHighBandPassFilter() {
        ...
        bandPass.frequency = 5000 // High target frequency for isolating upper frequencies.
        bandPass.bandwidth = 1
        bandPass.bypass = false
    }

    // Attempts to load an audio file from the application's resources. This is crucial for the playback and analysis to have content to process.
    ...
    // Installs an audio tap on the engine's output node. This tap allows for real-time audio analysis by capturing audio frames and processing them.
    ...
    // Calculate the root mean square (RMS) amplitude of the audio signal in the buffer. This represents the signal's power and is a measure of its loudness.
    ...
    // Map the amplitude value to a range between 0 and 1. This is useful for normalizing the amplitude for UI representation or further processing.
